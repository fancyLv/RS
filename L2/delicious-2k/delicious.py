...
# SimpleTagBased算法
def recommend_simpletag(user, N):
...
def recommend_normtag(user, N):
    recommend_items = defaultdict(int)
    tagged_items = user_items[user]
    wu = sum(t[1] for t in user_tags[user].items())
    for tag, wut in user_tags[user].items():
        wt = sum(t[1] for t in tag_items[tag].items())
        for item, wti in tag_items[tag].items():
            if item in tagged_items:
                continue
            recommend_items[item] += wut / wu * wti / wt
...
def recommend_tagbased_tfidf(user, N):
    ...
    for tag, wut in user_tags[user].items():
        wtu = len(tag_users[tag])
        for item, wti in tag_items[tag].items():
            if item in tagged_items:
                continue
            recommend_items[item] += wut / math.log10(1 + wtu) * wti
    return sorted(recommend_items.items(), key=operator.itemgetter(1), reverse=True)[0:N]


# 使用测试集，计算准确率和召回率
def precisionAndRecall(N, func=recommend_simpletag):
    hit = 0
    h_recall = 0
    h_precision = 0
...
def testRecommend():
    ...
    recommend_method = [recommend_simpletag, recommend_normtag, recommend_tagbased_tfidf]
    for func in recommend_method:
        print(func.__name__)
        for n in [5, 10, 20, 40, 60, 80, 100]:
            precision, recall = precisionAndRecall(n,func)
